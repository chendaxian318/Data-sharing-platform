...
struct st_arg
{
    ...
    char linktname[31];               // dblink指向的远程表名，如T_ZHOBTCODE1。
...
void _help()
{
    ...
    printf("不分批同步，把T_ZHOBTCODE1同步到T_ZHOBTCODE2。\n");
    printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/syncref /log/idc/syncref_ZHOBTCODE2.log "\
              "\"<localconnstr>idc/idcpwd</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset>"\
              "<linktname>T_ZHOBTCODE1</linktname><localtname>T_ZHOBTCODE2</localtname>"\
    ...
    printf("分批同步，把T_ZHOBTCODE1同步到T_ZHOBTCODE3。\n");
    printf("因为测试的需要，xmltodb程序每次会删除T_ZHOBTCODE1中的数据，全部的记录重新入库，keyid会变。\n");
    ...
    printf("       /project/tools/bin/procctl 10 /project/tools/bin/syncref /log/idc/syncref_ZHOBTCODE3.log "\
              "\"<localconnstr>idc/idcpwd</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset>"\
              "<linktname>T_ZHOBTCODE1</linktname><localtname>T_ZHOBTCODE3</localtname>"\
              "<remotecols>obtid,cityname,provname,lat,lon,height,upttime,keyid</remotecols>"\
              "<localcols>stid,cityname,provname,lat,lon,height,upttime,recid</localcols>"\
              "<rwhere>where obtid like '57%%%%'</rwhere>"\
              "<synctype>2</synctype><remoteconnstr>idc/idcpwd</remoteconnstr>"\
    ...
    printf("分批同步，把T_ZHOBTMIND1同步到T_ZHOBTMIND2。\n");
    printf("       /project/tools/bin/procctl 10 /project/tools/bin/syncref /log/idc/syncref_ZHOBTMIND2.log "\
              "\"<localconnstr>idc/idcpwd</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset>"\
              "<linktname>T_ZHOBTMIND1</linktname><localtname>T_ZHOBTMIND2</localtname>"\
    ...
    printf("linktname      dblink指向的远程表名，如T_ZHOBTCODE1。\n");
...
bool _syncref()
{
    ...
    //        select obtid,cityname,provname,lat,lon,height,upttime,keyid from T_ZHOBTCODE1 where obtid like '57%';
    if (starg.synctype==1)
    {
    ...
    //    select obtid,cityname,provname,lat,lon,height,upttime,keyid from T_ZHOBTCODE1 
